...
class DCplMatriks extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('d_cpl_matriks', function (Blueprint $table) {
            ...
            $table->string('cpl_kategori');
            $table->unsignedBigInteger('cpl_prodi_id')->nullable()->index();
            $table->unsignedBigInteger('cpl_sndikti_id')->nullable()->index();
            $table->unsignedBigInteger('pl_id')->nullable()->index();
            ...
            ->noActionOnDelete()
            ->noActionOnUpdate();
            $table->foreign(columns:'pl_id',name:'fk_cpl_matriks3')->references('pl_id')->on('m_profil_lulusan')
            ->noActionOnDelete()
            ->noActionOnUpdate();
